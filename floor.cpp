/**
 * @file floor.cpp
 * @brief Floor component management: displays, LEDs, buttons, keypad.
 */
...
int floor_init(floor_info floors[], size_t floor_num) {
  ...
  for (int i = 0; i < (int)_floors_size; i++) {
    _floors[i].upper7seg.begin(I2C_7SEG_BASE + i);
    if (_floors[i].def) start = i;
  }
  _lcd.init();
  _lcd.backlight();
  _leds.begin();
...
void floor_feedback(int current, const char* status) {
  ...
  if (!timer_elapsed(refresh, FLOOR_FEEDBACK_PERIOD)) return;
